...
fragment_set::fragment_set(const std::filesystem::path& set_log_path, size_t capacity,
                           global_data_view& storage, bool enable_replay)
    : m_storage(storage),
      m_set_log(set_log_path),
      m_lfu (capacity, [this](const auto& key, const auto loc) {
          for (auto[itr, rangeEnd] = m_hints.equal_range(key); itr != rangeEnd; ++itr)
              itr->second.reset();
          m_set.erase(loc);
      })
    {
...
fragment_set::response fragment_set::find(std::string_view data) {
    ...
    response resp {.hint = m_hints.emplace(res->pointer(), std::make_shared<decltype(res)>(res))};
    hint_lock.unlock();
...
void fragment_set::insert(
    const uint128_t& pointer, const std::string_view& data, const hint_type& hint) {
    ...
    std::weak_ptr<std::set<fragment_set_element>::const_iterator> set_hint = hint->second;
...
void fragment_set::mark_deduplication(const fragment& frag, const hint_type& hint) {
    std::lock_guard<std::shared_mutex> lock(m_mutex);
    m_hints.erase(hint);
