...
template <typename func, typename result = std::invoke_result<
                             func, std::size_t>::type::value_type>
requires (!std::is_same_v<result, void>)
coro<std::vector<result>> broadcast(boost::asio::io_context& ioc, func f, const
auto& services) { if (services.empty()) { throw std::runtime_error("no services
available");
    }
