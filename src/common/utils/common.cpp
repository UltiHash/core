...
static const std::map<uh::cluster::config_parameter, std::string>
    string_by_param = {
...
get_config_string(const uh::cluster::config_parameter& cfg_param) {
    if (auto search = string_by_param.find(cfg_param);
        search != string_by_param.end())
        return search->second;
    else
        throw std::invalid_argument("Invalid configuration parameter: " +
                                    std::to_string(cfg_param));
}
