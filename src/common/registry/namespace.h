...
constexpr std::array<std::pair<uh::cluster::etcd_service_attributes, const char*>, 5>
    string_by_config_parameter = {{
        ...
        {uh::cluster::STORAGE_WRITE_DURATION, "storage_write_duration"},
    }};
...
get_etcd_key_string(const uh::cluster::etcd_service_attributes& param) {
...
get_etcd_key_enum(const std::string& param) {
    for (const auto& entry : string_by_config_parameter) {
