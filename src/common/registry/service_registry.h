...
namespace uh::cluster {
    ...
    public:
        ...
        std::unique_ptr<registration> register_service(const server_config& config) {
            const std::string key_base = m_etcd_default_key_prefix + m_service_name + "/";
        ...
        std::vector<service_endpoint> get_service_instances(uh::cluster::role service_role) {
            ...
            const std::string service_prefix_path(m_etcd_default_key_prefix + get_service_string(service_role) + "/");
        ...
        void wait_for_dependency(uh::cluster::role dependency) {
            const std::string dependency_key(m_etcd_default_key_prefix + get_service_string(dependency));
    ...
    private:
        ...
        static constexpr std::size_t m_etcd_default_ttl = 5;
