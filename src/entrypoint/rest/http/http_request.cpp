...
http_request::http_request(
    const http::request_parser<http::empty_body>& recv_request,
    std::unique_ptr<URI> uri)
    : m_req(recv_request),
      m_method(get_http_method_from_beast(recv_request.get().base().method())),
      m_uri(std::move(uri)) {}

[[nodiscard]] std::map<std::string, std::string>
http_request::get_headers() const {
    std::map<std::string, std::string> headers_map;

    for (const auto& pair : m_req.get()) {
        headers_map[rest::utils::string_utils::to_lower(
            std::string{pair.name_string().data(), pair.name_string().length()}
                .c_str())] = pair.value();
    }
...
coro<void> http_request::read_body(boost::asio::ip::tcp::socket& stream,
    ...
    if (m_req.get().has_content_length()) {
        if (m_req.content_length().value() != 0) {
            ...
            auto size_transferred = co_await boost::asio::async_read(
                stream,
                boost::asio::buffer(m_body.data() + buffer.size(), data_left),
