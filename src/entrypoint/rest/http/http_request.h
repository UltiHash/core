...
class http_request {
  public:
    ...
    virtual coro<void> read_body(boost::asio::ip::tcp::socket& stream,
                                 boost::beast::flat_buffer& buffer);

    [[maybe_unused]] virtual inline void clear_body() { m_body.clear(); }

    [[nodiscard]] virtual inline std::size_t get_body_size() const {
        return m_body.size();
    }
    ...
    get_request_specific_headers() const = 0;

    [[nodiscard]] virtual inline const std::string& get_body() const {
        return m_body;
    }
