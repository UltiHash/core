...
delete_objects_request& delete_objects_request::operator=(
    const http::request_parser<http::empty_body>& recv_req) {
    ...
    if (payer_l != header_list.end() || payer_u != header_list.end()) {
        m_requestPayer = (payer_l != header_list.end()) ? payer_l->value()
                                                        : payer_u->value();
        m_requestPayerHasBeenSet = true;
    }

    const auto& bypassgovernance_l =
        header_list.find("x-amz-bypass-governance-retention");
    const auto& bypassgovernance_u =
        header_list.find("X-Amz-Bypass-Governance-Retention");
    if (bypassgovernance_l != header_list.end() ||
        bypassgovernance_u != header_list.end()) {
        m_bypassGovernanceRetention = (bypassgovernance_l != header_list.end())
                                          ? bypassgovernance_l->value()
                                          : bypassgovernance_u->value();
    ...
    if (checksum_l != header_list.end() || checksum_u != header_list.end()) {
        m_checksumAlgorithm = (checksum_l != header_list.end())
                                  ? checksum_l->value()
                                  : checksum_u->value();
...
delete_objects_request::get_request_specific_headers() const {
