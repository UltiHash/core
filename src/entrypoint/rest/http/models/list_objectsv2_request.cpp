...
list_objectsv2_request::list_objectsv2_request(
...
list_objectsv2_request& list_objectsv2_request::operator=(
    const http::request_parser<http::empty_body>& recv_req) {
    ...
    if (m_uri->query_string_exists("continuation-token")) {
        m_continuationToken =
            m_uri->get_query_string_value("continuation-token");
    ...
    const auto& optional_object_attributes_l =
        header_list.find("x-amz-optional-object-attributes");
    const auto& optional_object_attributes_u =
        header_list.find("X-Amz-Optional-Object-Attributes");
    if (optional_object_attributes_l != header_list.end() ||
        optional_object_attributes_u != header_list.end()) {
        m_optionalObjectAttributes.push_back(
            (optional_object_attributes_l != header_list.end())
                ? optional_object_attributes_l->value()
                : optional_object_attributes_u->value());
...
list_objectsv2_request::get_request_specific_headers() const {
