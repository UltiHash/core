...
list_objects_response::list_objects_response(const http_request& req)
    : http_response(req) {
    m_res.set(boost::beast::http::field::content_type, "application/xml");
    populate_response_headers();
}

list_objects_response::list_objects_response(
    const http_request& req, http::response<http::string_body> recv_res)
...
list_objects_response::get_response_specific_object() {
    ...
    if (m_prefixHasBeenSet && !m_prefix.empty()) {
        ...
        std::copy_if(m_contents.begin(), m_contents.end(),
                     std::back_inserter(filtered_by_prefix),
                     [&](const std::string& s) {
    ...
    if (m_contentsHasBeenSet && m_maxKeys != 0) {
        ...
        if (m_markerHasBeenSet) {
            auto index_itr = utils::string_utils::find_lexically_closest(
                m_contents, m_marker);
        ...
        for (; content_itr != m_contents.end(); content_itr++) {
            ...
            if (m_delimiterHasBeenSet &&
                ...
                content_xml_string +=
                    ...
                    "<Key>" +
                    (m_encodingTypeHasBeenSet
                         ? rest::utils::string_utils::URL_encode(*content_itr)
                         : *content_itr) +
                    "</Key>\n" + "</Contents>\n";
    ...
    if (m_delimiterHasBeenSet) {
        delimiter_xml_string =
            "<Delimiter>" +
            (m_encodingTypeHasBeenSet
                 ? rest::utils::string_utils::URL_encode(m_delimiter)
                 : m_delimiter) +
            "</Delimiter>\n";
    }
    ...
    if ((m_contents.size() - till_marker_count > m_maxKeys) && m_maxKeys != 0) {
        m_isTruncated = "true";
        next_marker_xml_string = "<NextMarker>" +
                                 m_contents[till_marker_count + m_maxKeys] +
                                 "</NextMarker>";
    }

    std::string max_keys_xml_string =
        "<MaxKeys>" + std::to_string(m_maxKeys) + "</MaxKeys>\n";

    std::string encoding_type_xml_string;
    if (m_encodingTypeHasBeenSet) {
        encoding_type_xml_string =
            "<EncodingType>" + m_encodingType + "</EncodingType>\n";
    }
    ...
    if (m_prefixHasBeenSet) {
        prefix_xml_string =
            "<Prefix>" +
            (m_encodingTypeHasBeenSet
                 ? rest::utils::string_utils::URL_encode(m_prefix)
                 : m_prefix) +
            "</Prefix>\n";
    }
    ...
    if (m_markerHasBeenSet) {
        //            (m_encodingTypeHasBeenSet ?
        //            rest::utils::string_utils::URL_encode(m_startAfter) :
        //            m_startAfter )
    ...
    set_body(std::string(
        ...
        next_marker_xml_string + content_xml_string + name_xml_string +
        prefix_xml_string + delimiter_xml_string + max_keys_xml_string +
        common_prefixes_xml_string + encoding_type_xml_string + key_count_xml +
        "</ListBucketResult>"));
