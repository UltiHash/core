...
delete_object_request& delete_object_request::operator=(
    const http::request_parser<http::empty_body>& recv_req) {
    ...
    if (mfa_l != header_list.end() || mfa_u != header_list.end()) {
        m_expectedBucketOwner =
            (mfa_l != header_list.end()) ? mfa_l->value() : mfa_u->value();
    ...
    if (request_payer_l != header_list.end() ||
        ...
        m_expectedBucketOwner = (request_payer_l != header_list.end())
                                    ? request_payer_l->value()
                                    : request_payer_u->value();
        m_expectedBucketOwnerHasBeenSet = true;
    }

    const auto& governance_l =
        header_list.find("x-amz-bypass-governance-retention");
    const auto& governance_u =
        header_list.find("X-Amz-Bypass-Governance-Retention");
    if (governance_l != header_list.end() ||
        governance_u != header_list.end()) {
        m_expectedBucketOwner = (governance_l != header_list.end())
                                    ? governance_l->value()
                                    : governance_u->value();
        m_expectedBucketOwnerHasBeenSet = true;
    }
    ...
    if (expec_l != header_list.end() || expec_u != header_list.end()) {
        m_expectedBucketOwner = (expec_l != header_list.end())
                                    ? expec_l->value()
                                    : expec_u->value();
        m_expectedBucketOwnerHasBeenSet = true;
    }
...
delete_object_request::get_request_specific_headers() const {
