...
create_bucket_request& create_bucket_request::operator=(
    const http::request_parser<http::empty_body>& recv_req) {
    ...
    if (acl_l != header_list.end() || acl_u != header_list.end()) {
        m_aCL = (acl_l != header_list.end())
                    ? get_bucket_canned_ACL_for_name(acl_l->value())
                    : get_bucket_canned_ACL_for_name(acl_u->value());
    ...
    if (grant_write_l != header_list.end() ||
        grant_write_u != header_list.end()) {
        m_grantRead = (grant_write_l != header_list.end())
                          ? grant_write_l->value()
                          : grant_write_u->value();
        m_grantReadHasBeenSet = true;
    }
    ...
    const auto& object_lock_enabled_l =
        header_list.find("x-amz-object-lock-mode");
    const auto& object_lock_enabled_u =
        header_list.find("X-Amz-Object-Lock-Mode");
    if (object_lock_enabled_l != header_list.end() ||
        object_lock_enabled_u != header_list.end()) {
        if (object_lock_enabled_u->value() == "true" ||
            object_lock_enabled_u->value() == "True" ||
            object_lock_enabled_l->value() == "true" ||
            object_lock_enabled_l->value() == "True")
    ...
    if (object_ownership_l != header_list.end() ||
        object_ownership_u != header_list.end()) {
        m_objectOwnership = (object_ownership_l != header_list.end())
                                ? object_ownership_l->value()
                                : object_ownership_u->value();
...
create_bucket_request::get_request_specific_headers() const {
