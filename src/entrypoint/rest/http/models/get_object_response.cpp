...
get_object_response::get_object_response(const http_request& req)
    : http_response(req) {}
...
get_object_response::get_response_specific_object() {
    ...
    if (m_contentDispositionHasBeenSet) {
        m_res.set(boost::beast::http::field::content_disposition,
    ...
    if (m_objectLockRetainUntilDateHasBeenSet) {
        m_res.set("x-amz-object-lock-retain-until-date",
