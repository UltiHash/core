...
list_multi_part_uploads_response::list_multi_part_uploads_response(
...
void list_multi_part_uploads_response::add_key_and_uploadid(
    const std::string& upload_id, const std::string& obj_name) {
...
list_multi_part_uploads_response::get_response_specific_object() {
    ...
    set_body(std::string("<ListMultipartUploadsResult>\n"
                         "   <Bucket>" +
                         m_orig_req.get_URI().get_bucket_id() + "</Bucket>\n" +
                         upload_xml_string + "</ListMultipartUploadsResult>"));
