...
complete_multi_part_upload_response::complete_multi_part_upload_response(
...
void complete_multi_part_upload_response::set_size(std::size_t size) {
    m_originalSize = size;
    m_originalSizeHasBeenSet = true;
}

void complete_multi_part_upload_response::set_effective_size(
    std::size_t effective_size) {
    m_effectiveSize = effective_size;
    m_effectiveSizeHasBeenSet = true;
}

void complete_multi_part_upload_response::set_space_savings(
...
complete_multi_part_upload_response::get_response_specific_object() {
    ...
    if (m_bucketKeyEnabledHasBeenSet) {
        m_res.set("x-amz-server-side-encryption-bucket-key-enabled",
                  m_bucketKeyEnabled);
    }
    ...
    if (m_originalSizeHasBeenSet) {
        ...
        m_res.set("uh-original-size-mb",
                  std::to_string(static_cast<double>(m_originalSize) /
                                 (1024 * 1024)));
    }
    ...
    if (m_effectiveSizeHasBeenSet) {
        ...
        m_res.set("uh-effective-size-mb",
                  std::to_string(static_cast<double>(m_effectiveSize) /
                                 (1024 * 1024)));
    }
    ...
    set_body(std::string("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
                         ...
                         "<Bucket>" +
                         m_orig_req.get_URI().get_bucket_id() +
                         "</Bucket>\n"
                         "<Key>" +
                         m_orig_req.get_URI().get_object_key() +
                         "</Key>\n"
                         "<ETag>" +
                         ((m_eTagHasBeenSet) ? m_eTag : "CustomEtag") +
                         "</ETag>\n"
                         "</CompleteMultipartUploadResult>"));
