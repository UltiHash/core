...
get_bucket_request& get_bucket_request::operator=(
    const http::request_parser<http::empty_body>& recv_req) {
    ...
    if (accountId_l != header_list.end() || accountId_u != header_list.end()) {
        m_accountId = (accountId_l != header_list.end()) ? accountId_l->value()
                                                         : accountId_u->value();
...
get_bucket_request::get_request_specific_headers() const {
