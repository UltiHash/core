...
list_buckets_response::list_buckets_response(const http_request& req)
    : http_response(req) {
...
list_buckets_response::get_response_specific_object() {
    ...
    set_body(std::string("<ListAllMyBucketsResult>\n"
                         "   <Buckets>\n" +
                         bucket_xml_string + "   </Buckets>\n" +
                         owner_xml_string + "</ListAllMyBucketsResult>"));
