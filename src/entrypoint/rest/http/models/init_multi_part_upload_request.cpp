...
init_multi_part_upload_request::init_multi_part_upload_request(
    ...
    bool inserted = server_state.m_uploads.insert_upload(
        upload_id, m_bucket_name, m_object_name);
    if (!inserted) {
...
init_multi_part_upload_request::get_request_specific_headers() const {
    return {};
}
