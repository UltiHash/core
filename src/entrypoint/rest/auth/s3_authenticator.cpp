...
// #include <iostream>
//
// #define EMPTY_SHA_256
// "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855" #define
// NEWLINE '\n'
//
...
//
//        if
//        (m_parsed_request.http_parsed_fields.find(http_fields::authorization)
//        == m_parsed_request.http_parsed_fields.end())
//            throw std::runtime_error("no authorization header");
//
//        std::string_view authorization_string =
//        m_parsed_request.http_parsed_fields[http_fields::authorization];
//
//        auto credential_index = authorization_string.find("Credential=");
//        auto signed_headers_index =
//        authorization_string.find("SignedHeaders="); auto signature_index =
//        authorization_string.find("Signature=");
//
//        if (credential_index == std::string::npos || signed_headers_index ==
//        std::string::npos || signature_index == std::string::npos)
...
//        {
//            auto slash_index = authorization_string.find_first_of('/',
//            index_pointer); if (slash_index == std::string::npos)
//                throw std::runtime_error("invalid authorization header");
//
//
//            switch (slashes)
//            {
//                case 0:
//                    m_access_key = authorization_string.substr(index_pointer,
//                    slash_index - index_pointer); break;
//                case 1:
//                    m_date = authorization_string.substr(index_pointer,
//                    slash_index - index_pointer); break;
//                case 2:
//                    m_region = authorization_string.substr(index_pointer,
//                    slash_index - index_pointer); break;
//                case 3:
//                    m_service = authorization_string.substr(index_pointer,
//                    slash_index - index_pointer); break;
//            }
//
...
//        auto semi_colon_index = authorization_string.find_first_of(";,",
//        index_pointer); while (semi_colon_index != std::string::npos)
//        {
//
//            m_signed_headers.emplace(authorization_string.substr(index_pointer,
//            semi_colon_index - index_pointer));
//
//            if (authorization_string[semi_colon_index] == ',') [[unlikely]]
//                break;
//
//            index_pointer = semi_colon_index + 1;
//            semi_colon_index = authorization_string.find_first_of(";,",
//            index_pointer);
//
//        }
//
...
//
//        if ( m_algorithm.empty() || m_access_key.empty() || m_date.empty() ||
//        m_region.empty()
//             || m_service.empty() || m_signed_headers.empty() ||
//             m_signature.empty())
//            throw std::runtime_error("invalid authorization header");
//
//
//        if
//        (m_parsed_request.http_parsed_fields.contains(http_fields::content_type))
//        {
//            // TODO: signed header is sorted alphabetically so if we don't
//            find content-type within 'c' character we can stop searching bool
//            contains_string = std::ranges::any_of(m_signed_headers,[](const
//            std::string& str)
//            {
//                return str == "content-type";
//            });
//            if (!contains_string)
//                throw std::runtime_error("content-type must also be included
//                in signed headers");
//        }
//
...
//                throw std::runtime_error("signed headers must include all the
//                x-amz tags given in the request");
//
//        }
//
//    }
...
//    {
//        return std::string('/' + m_parsed_request.bucket_id + '/' +
//        m_parsed_request.object_key + "\n");
//    }
//
////------------------------------------------------------------------------------
//
...
//            auto single_query_parameter =
//            m_parsed_request.query_string.substr(initial_index, query_index -
//            1);
//
//            if (!single_query_parameter.empty() &&
//            single_query_parameter.find('=') == std::string::npos)
...
//            SHA256((unsigned char *) payload.data(), payload.length(),
//            result); return {reinterpret_cast<char*>(result),
...
//    // not hex encoded
//    std::string
//    s3_authenticator::hmac_sha_256(const std::string& signing_key, const
//    std::string& string_to_sign) const
//    {
...
//
//        // TODO: what happens on multiple same headers, do we convert it to
//        one header with multiple values? for (const auto& header :
//        m_signed_headers)
//        {
...
//                if
//                (m_parsed_request.s3_parsed_fields.contains(converted_s3_enum))
//                {
//                    canonical_header_string += header + ":" +
//                    std::string(m_parsed_request.s3_parsed_fields[converted_s3_enum])
//                    + "\n";
//                }
//                else
//                    throw std::runtime_error("one of the given signed header
//                    doesn't exists");
//            }
//            else
//            {
...
//                if
//                (m_parsed_request.http_parsed_fields.contains(converted_http_enum))
//                {
//                    canonical_header_string += header + ":" +
//                    std::string(m_parsed_request.http_parsed_fields[converted_http_enum])
//                    + "\n";
//                }
//                else
//                    throw std::runtime_error("one of the given signed header
...
//            canonical_request += get_canonical_uri() +
//            get_canonical_query_string() + get_headers() + EMPTY_SHA_256;
//        else
//            canonical_request += get_canonical_uri() +
//            get_canonical_query_string() + get_headers() +
//            to_hex(sha_256(m_parsed_request.body));
//
...
//    {
//        return get_formatted_date() + '/' + m_region + '/' + m_service +
//        "/aws4_request";
//    }
//
////------------------------------------------------------------------------------
...
//    {
//        auto first_hmac = hmac_sha_256("AWS4" + m_secret_key,
//        get_formatted_date()); auto second_hmac = hmac_sha_256( first_hmac,
//        m_region ); auto third_hmac = hmac_sha_256( second_hmac, m_service );
...
//    {
//        auto calculated_signature = to_hex(hmac_sha_256( signing_key(),
//        get_string_to_sign() ));
//
