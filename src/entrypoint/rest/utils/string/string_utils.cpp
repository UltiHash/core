...
std::string string_utils::to_lower(const char* source) {
    ...
    std::transform(source, source + source_length, copy.begin(),
...
string_utils::find_lexically_closest(std::vector<std::string>& strings,
    ...
    if (nextDifferentItr != strings.end() && *nextDifferentItr == compareTo) {
        ++nextDifferentItr;
    } // TODO REVIEW THIS ONE MORE TIME, SOME TESTS FAIL BECAUSE OF REMOVING IT
      // BUT CONCEPTUALLY IT SHOULDN'T
