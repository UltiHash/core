...
namespace uh::cluster {
    ...
    void http_response::set_effective_size(std::size_t effective_size) {
        ...
        m_res.set("uh-original-size-mb", std::to_string(
                static_cast<double>(m_res.body().size()) / (1024 * 1024)));

        m_res.set("uh-effective-size", std::to_string(effective_size));
        m_res.set("uh-effective-size-mb", std::to_string(
                static_cast<double>(effective_size) / (1024 * 1024)));
    }
    ...
    const http::response<http::string_body>& http_response::get_prepared_response() {
    ...
    void http_response::set_etag(std::string etag) {
