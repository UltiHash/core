...
private:
    void report_stats() {
        ...
        LOG_INFO() << m_ctx.peer() << " retrieval size " << m_total << " b";
        LOG_INFO() << m_ctx.peer() << " retrieval duration " << duration.count() << " s";
        LOG_INFO() << m_ctx.peer() << " retrieval bandwidth " << bandwidth << " MB/s";
    }
