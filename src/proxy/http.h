...
template <typename Reader, typename Parser>
coro<void> async_read_header(const Reader& reader, Parser& parser) {
    auto header_size = std::vector<char>(reader->get_header_size());
...
coro<void> async_relay_buffer(Incomming& in, Outgoing& out,
