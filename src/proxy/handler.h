...
template <typename OutgoingStream> class handler : public protocol_handler {
public:
    explicit handler(std::unique_ptr<request_factory> factory,
                     std::function<std::unique_ptr<OutgoingStream>()> sf,
    ...
    coro<void> handle(boost::asio::ip::tcp::socket s) override {
        ...
        for (;;) {
            ...
            try {
                ...
                LOG_INFO() << peer
                           << ": incoming request: " << r.method_string() << " "
                           << r.target();
                ...
                if (r.method() == verb::head) {
                    ...
                    LOG_INFO()
                        << peer << ": relaying body of size " << *body_size;
...
private:
    ...
    std::function<std::unique_ptr<OutgoingStream>()> m_sf;
