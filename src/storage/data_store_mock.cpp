...
#define MAX_DATA_STORE_SIZE_BYTES (8 * MEBI_BYTE)
...
data_store::data_store(data_store_config conf,
    ...
    m_data.reserve(MAX_DATA_STORE_SIZE_BYTES);
...
std::size_t data_store::internal_delete(std::size_t offset, std::size_t size) {
    if (offset >= m_current_offset.load()) {
        LOG_WARN() << "attempted to delete data at the out-of-bounds offset="
                   << offset
                   << ", with last_page_offset=" << m_current_offset.load();
    ...
    std::fill(m_data.begin() + offset, m_data.begin() + offset + size, 0);
    m_used_space -= size;
    return size;
}
