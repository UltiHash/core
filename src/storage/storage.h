...
public:
    explicit storage(const service_config& service, const storage_config& sc)
        : m_etcd_client(service.etcd_url),
          ...
          m_storage(std::make_shared<local_storage>(m_service_id, sc.data_store, sc.data_store_count)),
          m_service_registry(STORAGE_SERVICE, m_service_id, m_etcd_client),
          m_server(
              sc.server,
              std::make_unique<storage_handler>(*m_storage),
              m_ioc) {}
    ...
    std::shared_ptr <local_storage> get_storage_interface () {
        return m_storage;
    }
