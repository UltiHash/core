...
int main() {
    ...
    // Create table
    const char* createTableSQL = "CREATE TABLE IF NOT EXISTS entries (hash "
                                 "BLOB(32), address BIGINT, metadata BIGINT);";
    ...
    const char* createIndexSQL =
        "CREATE INDEX IF NOT EXISTS hash_index ON entries(hash);";
    ...
    const char* insertSQL =
        "INSERT INTO entries (hash, address, metadata) VALUES (?, ?, ?);";
    ...
    if (rc != SQLITE_OK) {
        std::cerr << "Failed to prepare read statement: " << sqlite3_errmsg(db)
                  << std::endl;
        sqlite3_close(db);
        return 1;
    }
    ...
    std::cout << "Inserted " << numEntries << " entries in " << duration
    ...
    for (uint64_t i = 0; i < numEntries; i++) {
        ...
        sqlite3_bind_blob(readStmt, 1, sha256Hash, SHA256_DIGEST_LENGTH,
                          SQLITE_TRANSIENT);
    ...
    std::cout << "Read " << numEntries << " entries in " << duration
