...
void put_function(rocksdb::DB* db, uint64_t start, uint64_t end) {
    for (uint64_t i = start; i < end; ++i) {
        ...
        std::string key(reinterpret_cast<const char*>(sha256Hash), SHA256_DIGEST_LENGTH);
...
int main() {
    ...
    rocksdb::Status status = rocksdb::DB::Open(options, "/data/rocksdb_test", &db);
    if (!status.ok()) {
    ...
    for (uint64_t i = 0; i < numThreads; ++i) {
        ...
        put_threads.emplace_back(put_function, db, index_start, index_end); // Passes the thread ID to the function
    }
    ...
    std::cout << "Inserted " << numEntries << " entries " << duration << " milliseconds. Inserts per second: " << insertsPerSecond << std::endl;
    ...
    for (uint64_t i = 0; i < numThreads; ++i) {
        uint64_t index_start = i * entriesPerThread;
        uint64_t index_end = (i + 1) * entriesPerThread;
        get_threads.emplace_back(get_function, db, index_start, index_end); // Passes the thread ID to the function
    }
    ...
    std::cout << "Read " << numEntries << " entries " << duration << " milliseconds. Reads per second: " << readsPerSecond << std::endl;
