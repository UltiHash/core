...
class dedupe_set {
    ...
    [[nodiscard]] long get_match(const std::string_view& integration_data,
                                 size_t frag_size) const {

        if (integration_data.size() == frag_size or
            frag_size < min_fragment_size) {
            return 0;
        }
        ...
        while (frag_size + i >= min_fragment_size and -i < max_step_back) {
            ...
            const auto c =
                static_cast<uint8_t>(integration_data[step_back_index]);
  ...
  public:
    ...
    std::size_t dedupe(std::string_view& integration_data) {
        ...
        while (!integration_data.empty()) {
            ...
            if (static_cast<uint8_t>(integration_data[0]) >= mmax_char or
                static_cast<uint8_t>(integration_data[0]) < mmin_char) {
                break;
            }
            auto frag_size =
    ...
    void validate() const {
        for (const auto& f : dset) {
            if (static_cast<uint8_t>(f[0]) >= mmax_char or
                static_cast<uint8_t>(f[0]) < mmin_char) {
                throw;
            }
        }
    }
...
int main(int argc, char* args[]) {
    ...
    const auto range_size = static_cast<uint16_t>(
        std::ceil((std::numeric_limits<uint8_t>::max() + 1) /
                  static_cast<double>(set_count)));
    uint16_t offset = 0;
    for (size_t i = 0; i < set_count; ++i) {
        sets.emplace(offset,
                     dedupe_set{static_cast<uint8_t>(offset),
                                static_cast<uint16_t>(offset + range_size)});
        offset += range_size;
    }
    ...
    while (!integration_data.empty()) {
        ...
        const auto& s =
            sets.upper_bound(static_cast<uint8_t>(integration_data[0]));
        const auto prev_size = integration_data.size();
        const auto set_effective_size =
            std::prev(s)->second.dedupe(integration_data);
        const auto integrated_size = prev_size - integration_data.size();
        if (const auto f = space_savings.find(std::prev(s)->first);
            f == space_savings.cend()) {
            transfer_size += integration_data.size();
            space_savings.emplace_hint(f, std::prev(s)->first,
                                       integrated_size - set_effective_size);
        } else {
            f->second += integrated_size - set_effective_size;
        }
    ...
    for (const auto& set_space_saving : space_savings) {
        std::cout << "set " << i++ << " fragment count " << s->second.get_size()
                  << " space saving "
                  << static_cast<double>(set_space_saving.second) / GB << " GB"
                  << std::endl;
        s++;
    }
    ...
    std::cout << "transfer size: " << static_cast<double>(transfer_size) / GB
              << " GB" << std::endl;
    std::cout << "set switch count: " << switch_count << std::endl;
}