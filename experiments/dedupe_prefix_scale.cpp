...
public:
    ...
    std::size_t dedupe (std::string_view& integration_data) {
        ...
        while (!integration_data.empty()) {
            ...
            if (static_cast <uint8_t> (integration_data[0]) >= mmax_char or static_cast <uint8_t> (integration_data[0]) < mmin_char) {
                break;
            }
    ...
    void validate () const {
        for (const auto& f: dset) {
            if (static_cast <uint8_t> (f[0]) >= mmax_char or static_cast <uint8_t> (f[0]) < mmin_char) {
                throw;
            }
        }
    }
...
int main (int argc, char* args[]) {
    ...
    while (!integration_data.empty()) {
        ...
        const auto& s = sets.upper_bound(static_cast <uint8_t> (integration_data[0]));
        const auto prev_size = integration_data.size();
        const auto set_effective_size = std::prev (s)->second.dedupe(integration_data);
        ...
        if (const auto f = space_savings.find(std::prev (s)->first); f == space_savings.cend()) {
            ...
            space_savings.emplace_hint (f, std::prev (s)->first, integrated_size - set_effective_size);
        }
        else {
