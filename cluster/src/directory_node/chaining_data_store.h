...
class chaining_data_store {
    ...
    struct header {
        ...
        inline void load_from_file (const int fd) {
            uint32_t buf;
...
public:
    ...
    index_type write (std::span <char> data) {
        ...
        for (const auto& partial_alloc: alloc) {
            ...
            const auto header_size = (h.has_next) ? header::large_size: header::small_size;
    ...
    size_type read (char* buffer, index_type index) const {
...
private:
    ...
    alloc_t allocate (std::size_t size) {
        ...
        while (free_spot.has_value() and required_size > 0) {
            ...
            if (required_size > 0) {
                required_size += header::large_size;
            }
        ...
        while (required_size > 0) {
            partial_size = std::min (required_size, m_conf.max_file_size - last_file_data_end);
            ...
            if (partial_size == 0) [[unlikely]] {
                ...
                partial_size = std::min (required_size, m_conf.max_file_size);
