...
void entry_job::create_messengers() {
    ...
    for (const auto& dedupe_node: m_cluster_map.m_roles.at(DEDUPE_NODE)) {
        m_dedupe_nodes.emplace_back(m_io_service, dedupe_node.second, m_cluster_map.m_cluster_conf.dedupe_node_conf.server_conf.port);
    }

    for (const auto& phonebook: m_cluster_map.m_roles.at(PHONE_BOOK_NODE)) {
        m_phonebooks.emplace_back(m_io_service, phonebook.second, m_cluster_map.m_cluster_conf.phonebook_node_conf.server_conf.port);
    }
}
