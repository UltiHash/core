...
DECLARE
    ...
    bucket_id BIGINT;
BEGIN
    SELECT count(1), o.bucket_id INTO object_count, bucket_id
    FROM objects o
        LEFT JOIN object_status s ON o.id = s.object_id
    ...
    IF bucket_id IS NULL THEN
        RAISE EXCEPTION 'Cannot delete bucket "%", as it does not appear to exist.', bucket;
    END IF;

    INSERT INTO bucket_status (bucket_id, status)
    VALUES (bucket_id, status_deleted())
...
DECLARE bucket_id BIGINT;
        object_id BIGINT;
BEGIN
PERFORM uh_get_bucket_id(bucket) into bucket_id;

SELECT o.id INTO object_id FROM objects o WHERE o.bucket_id = bucket_id AND name = object;

IF object_id IS NOT NULL THEN
        INSERT INTO object_status (object_id, status)
        VALUES (object_id, status_deleted());
END IF;
