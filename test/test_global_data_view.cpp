...
BOOST_FIXTURE_TEST_CASE(invalid_read_fragment, global_data_view_fixture) {
    BOOST_CHECK_THROW(global_data_view().read_fragment(
                          std::numeric_limits<uint64_t>::max(), 8 * KIBI_BYTE),
                      std::runtime_error);
}
...
BOOST_FIXTURE_TEST_CASE(valid_write_read_fragment, global_data_view_fixture) {
    ...
    auto addr = global_data_view().write(input_buffer.get_str_view());
    BOOST_CHECK(input_buffer.size() == addr.data_size());
    BOOST_CHECK(addr.pointers.size() == 2);
    BOOST_CHECK(addr.sizes.size() == 1);
    global_data_view().sync(addr);

    auto frag = addr.first();
    auto result_buffer =
        global_data_view().read_fragment(frag.pointer, frag.size);
...
BOOST_FIXTURE_TEST_CASE(invalid_read_address, global_data_view_fixture) {
    ...
    BOOST_CHECK_THROW(
        global_data_view().read_address(result_buffer.data(), addr),
        std::runtime_error);
}
...
BOOST_FIXTURE_TEST_CASE(valid_write_read_address, global_data_view_fixture) {
    ...
    address addr;
    addr.append_address(global_data_view().write(
...
BOOST_FIXTURE_TEST_CASE(invalid_cached_sample, global_data_view_fixture) {
    auto sample =
        global_data_view().cached_sample(std::numeric_limits<uint64_t>::max());
...
BOOST_FIXTURE_TEST_CASE(valid_cached_sample, global_data_view_fixture) {
    ...
    auto frag = addr.first();
    auto short_sample = global_data_view().cached_sample(frag.pointer);
    BOOST_CHECK(short_sample.size() ==
                global_data_view().l1_cache_sample_size());
    BOOST_CHECK(input_buffer.get_str_view().substr(
                    0, global_data_view().l1_cache_sample_size()) ==
