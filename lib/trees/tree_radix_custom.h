...
//checking if children need to be generated before and after the found input peace, reference to data of tree required
                        ...
                        if constexpr (!reverse) {
                            if(std::get<2>(*match_beg)==this){
                                child_beg = std::vector<unsigned char>{};
                                std::copy(data.begin(),data.begin()+std::get<0>(*match_beg),child_beg.begin());
                                //child data sequence middle, reference data
                                child_mid = std::vector<unsigned char>{};
                                std::copy(data.begin()+std::get<0>(*match_beg),data.begin()+std::get<0>(*match_beg)+std::get<1>(*match_beg),child_mid.begin());
                                //child data sequence end, reference data
                                child_end = std::vector<unsigned char>{};
                                std::copy(data.begin()+std::get<0>(*match_beg)+std::get<1>(*match_beg)+1,data.end(),child_end.begin());
                            }
                            else{
                                child_beg = std::vector<unsigned char>{};
                            ...
                            if(std::get<2>(*match_beg)==this){
                                child_beg = std::vector<unsigned char>{};
                                std::copy(data.rbegin(),data.rbegin()+std::get<0>(*match_beg),child_beg.rbegin());
                                //child data sequence middle, reference data
                                child_mid = std::vector<unsigned char>{};
                                std::copy(data.rbegin()+std::get<0>(*match_beg),data.rbegin()+std::get<0>(*match_beg)+std::get<1>(*match_beg),child_mid.rbegin());
                                //child data sequence end, reference data
                                child_end = std::vector<unsigned char>{};
                                std::copy(data.rbegin()+std::get<0>(*match_beg)+std::get<1>(*match_beg)+1,data.rend(),child_end.rbegin());
                            }
                            else{
                                child_beg = std::vector<unsigned char>{};
