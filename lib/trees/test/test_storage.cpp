...
BOOST_AUTO_TEST_CASE(write_read_base_test)
{
    ...
    uh::trees::tree_storage t1(base_test,1);//A test folder reserved for tree storage
 ...
 * test setting up a tree storage at a given location
 */
BOOST_AUTO_TEST_CASE(multithreaded_constructor_test)
{
    //tests for any linux machine
    std::filesystem::path target = std::filesystem::path("/tmp");
...
BOOST_AUTO_TEST_CASE(multithreaded_write_read_base_test)
{
    //tests for any linux machine
    std::filesystem::path target = std::filesystem::path("/tmp");
    auto base_test = target / "0000";
...
BOOST_AUTO_TEST_CASE(multithreaded_write_read_test)
{
    //tests for any linux machine
    std::filesystem::path target = std::filesystem::path("/tmp");
    uh::trees::tree_storage t1(target);//A test folder reserved for tree storage
...
BOOST_AUTO_TEST_CASE(multithreaded_index_read_test)
{
    //get entire index, read block with local_block_reference and finally calculate the hash if it matches
    struct timeval time{};
    gettimeofday(&time, nullptr);
...
BOOST_AUTO_TEST_CASE(multithreaded_get_info_set_time_test)
{
    struct timeval time{};
    //tests for any linux machine
    std::filesystem::path target = std::filesystem::path("/tmp");
...
BOOST_AUTO_TEST_CASE(multithreaded_delete_test)
{
    //tests for any linux machine
    std::filesystem::path target = std::filesystem::path("/tmp");
    uh::trees::tree_storage t1(target);//A test folder reserved for tree storage
