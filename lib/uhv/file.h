...
public:
     ...
     */
    void serialize(const std::list<std::unique_ptr<uhv::f_meta_data>>& metadata);

    /**
     * Read the current contents of the UHV file and return it via `out_queue`.
     */
    void deserialize(uhv::job_queue<std::unique_ptr<uhv::f_meta_data>>& out_queue);

    /**
     * Read the current contents of the UHV file and return it in a list.
     */
    std::list<std::unique_ptr<uhv::f_meta_data>> deserialize();

    /**
     * Append a new entry to the end of the UHV file.
     */
    void append(std::unique_ptr<uhv::f_meta_data> md);
